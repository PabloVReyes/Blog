...
import { addRole, deleteRole, fecthRoles, updateRoles } from "../api";
...
export const useRolesStore = create<RolesState>((set, get) => ({
    ...
    async fetch() {
        try {
            ...
            const { data, meta } = await fecthRoles({ page, limit, search })
    ...
    async update(id, data) {
        try {
            const newData = await updateRoles(String(id), data)
            set((state) => ({
